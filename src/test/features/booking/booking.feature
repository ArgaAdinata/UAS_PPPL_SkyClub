Feature: Pemesanan Lapangan oleh Penyewa
  ...
  Background:
    ...
    And Pengguna berada pada halaman detail lapangan
  ...
  Scenario: Pemesanan berhasil dengan saldo cukup
    ...
    And Slot penyewaan pada waktu tersebut tidak tersedia untuk pengguna lain
  ...
  Scenario: Pemesanan gagal karena saldo tidak cukup
    ...
    And Slot penyewaan tetap tersedia

  @requires_login
  Scenario: Pemesanan gagal karena tidak memilih jadwal
    Given Pengguna memiliki saldo yang cukup
    When Pengguna tidak memilih jadwal apapun
    And Pengguna mencoba menuju ke halaman pembayaran
    Then Sistem menampilkan pesan error "Pilih jadwal terlebih dahulu"
